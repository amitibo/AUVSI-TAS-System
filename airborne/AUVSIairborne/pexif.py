...
class IfdMPImage(IfdData):
    name = "MPF"

    tags = {
        0xB000: ("MPF Version", "MPFVersion", LONG),
        0xB001: ("Number of MP images", "MPImageCount", LONG),
        0xB002: ("MP Image list", "MPImageList", UNDEFINED),
    }

    #def special_handler(self, tag, data):
    #    if tag in self.tags and self.tags[tag][1] == "MPImageList":
    #        self.exif_file.make = data.strip('\0')
    ...
    def ifd_handler(self, data):
        ...
        for (tag, exif_type, val) in self.entries:
            if tag == 0xB002:
                images.extend(self.read_images(val, data))

        object.__setattr__(self, 'preview', images)

    def read_images(self, data, ifd_data):
        images = []
        offset = 0

        for i in range(self.MPImageCount[0]):
            image = self.read_image(data[offset:offset+16], ifd_data)
    ...
    def read_image(self, data, ifd_data):
        flags, length, start, dep1, dep2 = unpack(self.e + 'IIIHH', ''.join(data))

        if length == 0 or start == 0:
            return None

        return self.exif_file.read_ahead(start, length) #ifd_data[start:start+length]
...
class App2Segment(DefaultSegment):
    def __init__(self, marker, fd, data, mode):
        self.ifds = []
        self.e = '<'
        self.tiff_endian = 'II'
    ...
    def write(self, fd):
        # Do not write this segment as we can't recalculate offsets
        pass

    def read_ahead(self, offset, size):
        offset += 4 # 'MPF\0'
        ...
        #print 'Reading', size, 'bytes at offset', offset
        self.fd.seek(offset, 1)
        data = self.fd.read(size)
        self.fd.seek(-offset - len(data), 1)

        #print 'len=', len(data), 'expected=', size
        #print ' '.join(hex(ord(x)) for x in data[0:16])
        return data
    ...
    def parse_data(self, data):
        """Overloads the DefaultSegment method to parse the data of
        this segment. Can raise InvalidFile if we don't get what we expect."""
        mpf = unpack("4s", data[:4])[0]
        mpf = mpf.strip('\0')

        if (mpf != "MPF"):
            return # Ignore the segment
        ...
        while offset:
            ...
            start = 2 + offset + (num_entries*12)
            if (count == 1):
                ifd = IfdMPImage(self.e, offset, self, self.mode, tiff_data)
            else:
                raise JpegFile.InvalidFile()
            self.ifds.append(ifd)
    ...
    def dump(self, fd):
        print >> fd, " Section: [ APP2] Size: %6d" % (len(self.data))
        for ifd in self.ifds:
            ifd.dump(fd)
...
class JpegFile:
    ...
    def get_app2(self):
        for segment in self._segments:
            if isinstance(segment, App2Segment):
                return segment
        return None

    def _get_app2(self):
        """APP2 Attribute property"""
        app2 = self.get_app2()
#        if app2 is None:
#            raise AttributeError
        return app2
